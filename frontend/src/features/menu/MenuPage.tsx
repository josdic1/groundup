    ...
    <main className="menu-page">
      ...
      <section className="menu-add-card">
        ...
        <div className="menu-add-grid">
          ...
          <select
            value={newItem.soldBy}
            onChange={(e) => setNewItem({ ...newItem, soldBy: e.target.value as SoldBy })}
          >
            ...
            {items.map((item) => {
              ...
              return (
                <div className={`menu-row ${!current.isActive ? 'inactive' : ''}`} key={item.id}>
                  <input
                    value={current.name}
                    onChange={(e) => updateDraft(item, { name: e.target.value })}
                  />
                  ...
                  <input
                    ...
                    step="0.01"
                    value={current.price}
                    onChange={(e) => updateDraft(item, { price: Number(e.target.value) })}
                  />

                  <input
                    value={current.unit}
                    onChange={(e) => updateDraft(item, { unit: e.target.value })}
                  />

                  <select
                    value={current.soldBy}
                    onChange={(e) => updateDraft(item, { soldBy: e.target.value as SoldBy })}
                  >
                    <option value="weight">weight</option>
                    <option value="each">each</option>
                  </select>
                  ...
                  <button
                    type="button"
                    onClick={() => updateDraft(item, { isActive: !current.isActive })}
                  >
                    {current.isActive ? 'Active' : 'Inactive'}
                  </button>

                  <button type="button" disabled={!changed} onClick={() => save(item)}>
                    Save
                  </button>
                </div>
              );
